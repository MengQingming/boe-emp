...
<script type="text/javascript"> 
	...
	var menuTreeselectNodes=[
			<c:forEach items="${fns:getMenuList()}" var="menu"><c:if test="${menu.displayFlag eq '1'}">{id:"${menu.id}", pId:"${not empty menu.parent.id ? menu.parent.id : 0}", name:"<i class=\"icon-${not empty menu.icon ? menu.icon : 'file'}\"></i>&nbsp;${not empty menu.parent.id ? menu.menuName : ''}", url:"${not empty menu.url && fn:indexOf(menu.url, '://') eq -1 ? ctx : ''}${not empty menu.url ? menu.url : 'javascript:'}", target:"${not empty menu.target ? menu.target:'_self'}"},
