...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/sys/group/list?id=${group.parent.id}&parentIds=${group.parentIds}">机构详情</a></li>
		<li><a href="${ctx}/sys/group/form?id=${group.id}&parent.id=${group.parent.id}">机构<shiro:hasPermission name="sys:group:edit">${not empty group.id?'添加':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:group:edit">查看</shiro:lacksPermission></a></li>
	</ul>
	<form:form id="inputForm" modelAttribute="group" action="${ctx}/sys/group/save" method="post" class="form-horizontal" style="margin:0px;">
