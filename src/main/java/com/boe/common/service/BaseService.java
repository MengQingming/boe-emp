...
import com.boe.common.persistence.BaseEntity;
				...
				for (String oa : StringUtils.split(officeAlias, ",")){
					if (!dataScope.contains(r.getDataScope()) && StringUtils.isNotBlank(oa)){
						if (Role.DATA_SCOPE_ALL.equals(r.getDataScope())){
							...
							sqlString.append(" OR " + oa + ".id = '" + user.getGroup().getId() + "'");
							sqlString.append(" OR " + oa + ".parent_ids LIKE '" + user.getGroup().getParentIds() + user.getGroup().getId() + ",%'");
						}
						else if (Role.DATA_SCOPE_COMPANY.equals(r.getDataScope())){
							sqlString.append(" OR " + oa + ".id = '" + user.getGroup().getId() + "'");
							// 包括本公司下的部门 （type=1:公司；type=2：部门）
							sqlString.append(" OR (" + oa + ".parent_id = '" + user.getGroup().getId() + "' AND " + oa + ".type = '2')");
						}
	...
	public static void dataScopeFilter(BaseEntity<?> entity, String sqlMapKey, String officeWheres, String userWheres) {
		...
		for (String where : StringUtils.split(officeWheres, ",")){
			if (Role.DATA_SCOPE_COMPANY_AND_CHILD.equals(dataScopeString)){
				// 包括本公司下的部门 （type=1:公司；type=2：部门）
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_GROUP");
				sqlString.append(" WHERE type='2'");
				sqlString.append(" AND (id = '" + user.getGroup().getId() + "'");
				sqlString.append(" OR parent_ids LIKE '" + user.getGroup().getParentIds() + user.getGroup().getId() + ",%')");
				...
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_GROUP");
				sqlString.append(" WHERE (id = '" + user.getGroup().getId() + "'");
				sqlString.append(" OR parent_ids LIKE '" + user.getGroup().getParentIds() + user.getGroup().getId() + ",%')");
				sqlString.append(" AND " + where +")");
			}
				...
				sqlString.append(" AND EXISTS (SELECT 1 FROM SYS_ROLE_GROUP ro123456, SYS_GROUP o123456");
				sqlString.append(" WHERE ro123456.group_id = o123456.id");
