...
import com.boe.common.mapper.JaxbMapper;
import com.boe.common.utils.DateUtils;
import com.boe.common.utils.FileUtils;
import com.boe.common.utils.FreeMarkers;
...
import com.boe.gen.entity.GenCategory;
import com.boe.gen.entity.GenConfig;
import com.boe.gen.entity.GenScheme;
import com.boe.gen.entity.GenTable;
import com.boe.gen.entity.GenTableColumn;
import com.boe.gen.entity.GenTemplate;
...
public class GenUtils {
	...
	public static void initColumnField(GenTable genTable){
		for (GenTableColumn column : genTable.getColumnList()){
			...
			// 如果是不是新增列，则跳过。
			if (null!=column.getId() && column.getId()>0){
				continue;
			}
			...
			// 部门
				...
				column.setJavaType(Group.class.getName());
				column.setJavaField(column.getJavaField().replaceAll("Id", ".id|name"));
...
//		}
	...
	public static List<GenTemplate> getTemplateList(GenConfig config, String category, boolean isChildTable){
		...
		if (config !=null && config.getCategoryList() != null && category !=  null){
			for (GenCategory e : config.getCategoryList()){
				if (category.equals(e.getDictCode())){
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme){
		...
		model.put("functionAuthor", StringUtils.isNotBlank(genScheme.getFunctionAuthor())?genScheme.getFunctionAuthor(): UserUtils.getUser().getFullName());
